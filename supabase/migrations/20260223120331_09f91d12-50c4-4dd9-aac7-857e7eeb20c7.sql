...
-- Create a function that triggers sync-and-snapshot for all spaces with configured ad accounts
...
BEGIN
  ...
  -- Iterate over all spaces that have meta_id or tiktok_id configured
  FOR space_record IN 
    SELECT id FROM spaces 
    WHERE meta_id IS NOT NULL OR tiktok_id IS NOT NULL
  LOOP
    -- Call sync-and-snapshot edge function for each space
    PERFORM extensions.http_post(
      url := supabase_url || '/functions/v1/sync-and-snapshot',
      body := json_build_object('spaceId', space_record.id)::text,
